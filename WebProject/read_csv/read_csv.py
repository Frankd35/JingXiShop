...
#读取数据
test_data = pd.read_csv(r'D:\JXproject\WebProject\read_csv\1.csv')
test_data.head()
#连接数据库
...
def insert_test_data():
    ...
    for i in range(0,test_data.shape[0]):
        ...
        line_data = test_data.iloc[i]
         #读取第每行中每列数据
        id = i+19  # ← 与num的初始值相同，每次需要修改
        # 第[1]项为目录id，每次 ↓ 录入不同目录需要修改其值
        value = (str(id),str(1),str(line_data[3]),str(line_data[2]),str(line_data[5]),str(line_data[4]),str("地球高质量商品"),
