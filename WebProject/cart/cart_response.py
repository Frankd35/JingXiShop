...
class ShowGoods:
    def __init__(self, gid, name, img, num, price, shopName, max_num, tttprice=0):
        self.gid = gid
        self.name = name
        self.img = img
        self.num = num
...
def dealRequest(user_id, flag, gid, isChosen):
    ...
    if flag == 0:
        ...
        if isChosen:
            ...
            print("取消全选")
        return []
    elif flag == 6: # 更新购物车商品数量 -> 此时isChosen表征购物车商品数量
        if isChosen < 0:
            print("没有取到num值")
            return []
        cart_model.Cart.objects.filter(user_id=user_id, goods_id=gid).update(goods_num=isChosen)
        print("购物车商品数量更新")
        return []
    for i in tempList:
        ...
        hhh = ShowGoods(i.goods_id, tempName, tempImg, i.goods_num, tempPrice, tempShopName, tempGoods.number, total_price)
